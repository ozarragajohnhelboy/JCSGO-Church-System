        ...
        tableBtnGroups.forEach(function(group) {
            ...
            toggle.addEventListener('click', function(e) {
                ...
                if (group.classList.contains('show')) {
                    ...
                    const totalDropdownHeight = actions.length * buttonHeight;
                    
                    let startTop = Math.max(navbarBottom + 5, rect.bottom + 5);
                    
                    if (availableSpace < totalDropdownHeight) {
                        startTop = Math.max(navbarBottom + 5, footerTop - totalDropdownHeight - 10);
                    }
                    
                    actions.forEach(function(action, index) {
                        action.style.position = 'fixed';
                        action.style.top = (startTop + (index * buttonHeight)) + 'px';
                        action.style.right = '0.5rem';
                        action.style.left = 'auto';
                        action.style.bottom = 'auto';
                        action.style.width = '44px';
                        action.style.minWidth = '44px';
                        action.style.maxWidth = '44px';
                        action.style.height = '44px';
                        action.style.minHeight = '44px';
                        action.style.maxHeight = '44px';
                    });
                } else {
                    const actions = group.querySelectorAll('.btn-action');
                    actions.forEach(function(action) {
